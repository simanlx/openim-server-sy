...
func (d *DataBases) DelMsgByIndex(suffixUserID string, msg *crazy_server_sdk.MsgData, operationID string, seqIndex int) error {
...
func (d *DataBases) ReplaceMsgBySeq(uid string, msg *crazy_server_sdk.MsgData, operationID string) error {
...
func (d *DataBases) GetMsgBySeqIndex(uid string, seq uint32, operationID string) (*crazy_server_sdk.MsgData, error) {
	ctx, _ := context.WithTimeout(context.Background(), time.Duration(config.Config.Mongo.DBTimeout)*time.Second)
	c := d.mongoClient.Database(config.Config.Mongo.DBDatabase).Collection(cChat)
	...
	var msg crazy_server_sdk.MsgData
...
func (d *DataBases) ReplaceMsgByIndex(uid string, msg *crazy_server_sdk.MsgData, index int) error {
	ctx, _ := context.WithTimeout(context.Background(), time.Duration(config.Config.Mongo.DBTimeout)*time.Second)
	c := d.mongoClient.Database(config.Config.Mongo.DBDatabase).Collection(cChat)
	uid = getSeqUid(uid, msg.Seq)
	seqIndex := getMsgIndex(msg.Seq)
...
func (d *DataBases) GetMsgBySeqList(uid string, seqList []uint32, operationID string) (seqMsg []*crazy_server_sdk.MsgData, err error) {
	...
	for seqUid, value := range m {
		...
		for i := 0; i < len(sChat.Msg); i++ {
			msg := new(crazy_server_sdk.MsgData)
...
func (d *DataBases) ReplaceMsgToBlankByIndex(suffixID string, index int) (replaceMaxSeq uint32, err error) {
	...
	for i, msg := range userChat.Msg {
		if i <= index {
			msgPb := &crazy_server_sdk.MsgData{}
			if err = proto.Unmarshal(msg.Msg, msgPb); err != nil {
				continue
			}
			newMsgPb := &crazy_server_sdk.MsgData{Seq: msgPb.Seq}
...
func (d *DataBases) GetNewestMsg(ID string) (msg *crazy_server_sdk.MsgData, err error) {
	ctx, _ := context.WithTimeout(context.Background(), time.Duration(config.Config.Mongo.DBTimeout)*time.Second)
	c := d.mongoClient.Database(config.Config.Mongo.DBDatabase).Collection(cChat)
	regex := fmt.Sprintf("^%s:", ID)
...
func (d *DataBases) GetOldestMsg(ID string) (msg *crazy_server_sdk.MsgData, err error) {
	ctx, _ := context.WithTimeout(context.Background(), time.Duration(config.Config.Mongo.DBTimeout)*time.Second)
	c := d.mongoClient.Database(config.Config.Mongo.DBDatabase).Collection(cChat)
	regex := fmt.Sprintf("^%s:", ID)
	findOpts := options.Find().SetLimit(1).SetSort(bson.M{"uid": 1})
...
func (d *DataBases) GetMsgBySeqListMongo2(uid string, seqList []uint32, operationID string) (seqMsg []*crazy_server_sdk.MsgData, err error) {
	var hasSeqList []uint32
	singleCount := 0
	ctx, _ := context.WithTimeout(context.Background(), time.Duration(config.Config.Mongo.DBTimeout)*time.Second)
	c := d.mongoClient.Database(config.Config.Mongo.DBDatabase).Collection(cChat)
...
func (d *DataBases) GetSuperGroupMsgBySeqListMongo(groupID string, seqList []uint32, operationID string) (seqMsg []*crazy_server_sdk.MsgData, indexes map[uint32]int, err error) {
	var hasSeqList []uint32
...
func (d *DataBases) GetMsgAndIndexBySeqListInOneMongo2(suffixUserID string, seqList []uint32, operationID string) (seqMsg []*crazy_server_sdk.MsgData, indexList []int, unexistSeqList []uint32, err error) {
	ctx, _ := context.WithTimeout(context.Background(), time.Duration(config.Config.Mongo.DBTimeout)*time.Second)
	c := d.mongoClient.Database(config.Config.Mongo.DBDatabase).Collection(cChat)
	sChat := UserChat{}
...
func genExceptionMessageBySeqList(seqList []uint32) (exceptionMsg []*crazy_server_sdk.MsgData) {
...
func genExceptionSuperGroupMessageBySeqList(seqList []uint32, groupID string) (exceptionMsg []*crazy_server_sdk.MsgData) {
	for _, v := range seqList {
		msg := new(crazy_server_sdk.MsgData)
		msg.Seq = v
		msg.GroupID = groupID
